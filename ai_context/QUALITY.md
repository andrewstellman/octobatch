...
### Scenario 9: Expression Step Validation Enforcement (The Blackjack Verification Bug)

**What happened:** The Blackjack pipeline added a `verify_hand` expression step with a validation rule checking `verification_passed == True`. The expression correctly computed `verification_passed=False` on 456 of 900 units, but the orchestrator never enforced the rule — expression steps bypassed the validation pipeline entirely. All 456 mathematically incorrect hands flowed through unchecked. Three independent AI advisors all assumed validation rules applied universally to all step types. TOOLKIT.md documented the pattern as correct. The `ZeroDivisionError` hack that Claude Code originally used was the only approach that actually worked, but was "fixed" into the non-functional validation rule pattern.

**The requirement:** When an expression step has a `validation:` entry in `config.yaml`, the orchestrator must run the same schema and business logic validation on its output that LLM steps use. Expression steps without a `validation:` entry continue to work as before.

**How to verify:** Create a pipeline with an expression step that outputs a boolean field set to `False`. Add a validation rule checking that field. Run the pipeline. Verify the unit fails validation and is retried, not passed through silently.
