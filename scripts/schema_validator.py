...
def _coerce_value(value: Any, expected_type: str, path: str) -> tuple[Any, bool]:
    ...
    if expected_type == "integer":
        ...
        if isinstance(value, float):
            # Non-finite floats must flow into structured validation errors,
            # not raw Python exceptions from int(NaN/Inf).
...
def validate_line(
    ...
    # Reject non-finite floats explicitly (NaN/Infinity), which are not
    # valid JSON numbers for pipeline outputs and can silently corrupt data.
    def _find_non_finite_numbers(obj: Any, path: str = "$") -> list[dict]:
        found = []
        if isinstance(obj, float) and not math.isfinite(obj):
            found.append({
                "path": [path],
                "rule": "schema_non_finite_number",
                "message": f"Non-finite number at {path}: {obj!r}",
                "validator": "number",
            })
            return found
        if isinstance(obj, dict):
            for key, value in obj.items():
                found.extend(_find_non_finite_numbers(value, f"{path}.{key}"))
        elif isinstance(obj, list):
            for idx, value in enumerate(obj):
                found.extend(_find_non_finite_numbers(value, f"{path}[{idx}]"))
        return found

    non_finite_errors = _find_non_finite_numbers(data)

    # Validate against schema
    errors = list(validator.iter_errors(data))

    if errors or non_finite_errors:
        # Return data along with errors for retry support
        return data, non_finite_errors + format_all_errors(errors)
