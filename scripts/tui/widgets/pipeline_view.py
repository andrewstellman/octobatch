...
def render_pipeline_boxes(
    ...
    Args:
        ...
        step_funnel: Optional dict of {step_name: (resolved_count, input_count)} for funnel view.
                     resolved_count = valid + permanently_failed (all terminal outcomes).
                     When provided, each step shows its own throughput instead of global progress.
                     The checkered flag appears when resolved_count == input_count.
    ...
    for i, step in enumerate(steps):
        ...
        if step_funnel and step.name in step_funnel:
            funnel_resolved, funnel_input = step_funnel[step.name]
            if funnel_resolved == funnel_input and funnel_input > 0:
    ...
    for i, step in enumerate(steps):
        ...
        if step_funnel and step.name in step_funnel:
            funnel_resolved, funnel_input = step_funnel[step.name]
            if funnel_resolved == 0 and funnel_input == 0:
                progress = "--/--"
            else:
                progress = _format_progress(f"{funnel_resolved}/{funnel_input}")
        else:
