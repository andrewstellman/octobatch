...
"""
        ...
        if self.run_data.steps and self.selected_step_index < len(self.run_data.steps):
            ...
            # Derive processing/pending count using step-specific input, not
            # run-wide total.  Later steps receive fewer inputs when earlier
            # steps have permanent failures.  Use unique permanently-failed
            # unit count (not raw failure-file entries which double-count
            # retries) so the number reflects actual unresolved units.
            step_idx = self.selected_step_index
            if step_idx == 0:
                step_input = total
            else:
                prev_step = self.run_data.pipeline[step_idx - 1]
                step_input = self._count_step_valid(prev_step)
            permanently_failed = self._count_step_permanently_failed(step.name)
            processing = max(0, step_input - passed - permanently_failed)
...
"""
    ...
    def _render_pipeline_content(self) -> str:
        ...
        # Build funnel data: per-step resolved counts from on-disk files.
        # "Resolved" = valid + permanently failed (units that reached a terminal
        # state). This shows each step's own throughput instead of global
        # progress, making bottlenecks immediately visible.  The checkered flag
        # appears when resolved == input (all units accounted for).
        step_funnel = {}
        step_valid_counts = {}
        ...
        for i, step_name in enumerate(pipeline):
            ...
            perm_failed = self._count_step_permanently_failed(step_name)
            step_valid_counts[step_name] = valid
            if i == 0:
                step_input = total_units
            else:
                step_input = step_valid_counts[pipeline[i - 1]]
            step_funnel[step_name] = (valid + perm_failed, step_input)
    ...
    def _count_step_permanently_failed(self, step_name: str) -> int:
        """Count unique units that permanently failed at a step.

        Scans all chunk dirs (including retry_*). A unit is permanently
        failed if it appears in a failures file but NOT in any validated
        file for the same step.
        """
        run_dir = self.run_data.run_dir
        chunks_dir = run_dir / "chunks"
        if not chunks_dir.exists():
        ...
        # Collect all validated unit_ids for this step across ALL chunk dirs
        ...
        # Collect unique failed unit_ids NOT in validated set
                ...
                try:
                    with _open_jsonl(failures_path) as f:
                        for line in f:
                            ...
                            try:
                                failure = json.loads(line)
                                uid = failure.get("unit_id")
                                if uid and uid not in validated_ids:
                                    failed_ids.add(uid)
                            except json.JSONDecodeError:
                                continue
                except Exception:
