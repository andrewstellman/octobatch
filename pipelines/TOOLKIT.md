...
- No schema validation by default (expressions write directly to validated output). If a `validation:` entry exists for the step, schema and business logic rules are enforced — see [Failing Expression Steps with Validation Rules](#failing-expression-steps-with-validation-rules).
...
> **asteval limitation: no generator expressions.** The expression evaluator uses asteval, which does not support generator expressions. Always use list comprehensions inside `sum()`, `min()`, `max()`, `len()`, and other functions that accept iterables.
>
> ```python
> # WRONG — generator expression will fail
> sum(val for val in items)
> sum(1 for c in cards if c == 'A')
>
...
### Failing Expression Steps with Validation Rules

Expression steps can act as deterministic verification gates — computing whether some condition holds and failing the unit cleanly if it doesn't. The pattern is: **output a boolean field from the expression step, then add a validation rule that checks it.**

> **Do NOT use intentional exceptions for control flow.** Never write expressions like `1 / int(condition)` to trigger ZeroDivisionError on failure. Deliberately crashing an expression pollutes logs with Python stack traces and makes the system look broken. Always use the validation framework for clean, transparent failure handling.

**Pattern: Expression step + validation rule**
...
pipeline:
  steps:
    ...
    - name: verify
      scope: expression
      description: "Verify the generated content meets requirements"
      expressions:
        word_count: "len(content.split())"
        has_required_sections: "all(s in content for s in ['Introduction', 'Conclusion'])"
        verification_passed: "word_count >= 100 and has_required_sections"
        verification_details: "f'word_count={word_count}, has_required_sections={has_required_sections}'"

    - name: refine
      prompt_template: refine.jinja2
      description: "Refine the content"
```
...
validation:
  verify:
    ...
    rules:
      - name: verification_must_pass
        expr: "verification_passed == True"
        error: "Verification failed: {verification_details}"
        level: error
```

**How it works:**

1. The expression step computes a `verification_passed` boolean (and optionally a `verification_details` string for diagnostics)
2. The `validation:` entry for the expression step checks the boolean with a rule
3. If the rule fails, the unit is marked as a **validation failure** — it appears as a yellow ⚠ in the TUI, not a red ✗
4. With `validation_retry.max_attempts` configured, the unit is automatically retried from the beginning of the pipeline
5. The error message includes `{verification_details}` for easy debugging

**Why this is better than exceptions:**

| Approach | Log output | TUI display | Retryable? |
|----------|-----------|-------------|------------|
| Validation rule | Clean error: "Verification failed: word_count=50" | Yellow ⚠ validation failure | Yes |
| Intentional exception | Python traceback: "ZeroDivisionError: division by zero" | Red ✗ hard failure | No |

**Real-world example: Blackjack hand verification**

An expression step recalculates card totals from the action log and verifies the LLM's math:

```yaml
- name: verify_hand
  scope: expression
  description: "Verify hand totals and outcome"
  expressions:
    _player_calc: "<expression that recalculates player total from cards>"
    _dealer_calc: "<expression that recalculates dealer total from cards>"
...
When the LLM miscalculates a total, the unit fails cleanly with a message like: *"Hand verification failed: Player: calc=18 reported=19, Dealer: calc=17 reported=17"* — and gets retried automatically.
